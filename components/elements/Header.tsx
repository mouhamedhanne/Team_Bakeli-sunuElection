import Illustration_voter from "@/public/images/illustration-voter.svg";
import Image from "next/image";

export default function Header() {
  return (
    <div className="mt-16 lg:flex justify-between items-end">
      <div>
        <div className="flex justify-center lg:justify-start">
          <span
            className="inline-flex items-center mb-6 rounded-md font-medium 
            forced-colors:outline bg-color_secondary/15 text-color_primary 
             dark:bg-orange-500/10 dark:text-orange-400 
            px-1.5 py-0.5 text-sm/5 sm:text-[16px] gap-x-1.5"
          >
            Votez en toute transparence
          </span>
        </div>
        <h1
          className="uppercase text-5xl lg:text-6xl leading-[3.5rem] text-center lg:text-left font-extrabold 
          text-color_primary"
        >
          votre{" "}
          <b className="relative inline-block">
            <svg
              className="absolute z-0 h-[100%] w-[calc(100%+1rem)] fill-color_secondary"
          ...
          depend
          <br className="hidden lg:block" /> de vous
        </h1>
        <p
          className="text-text_muted mt-10 text-[20px] text-center lg:text-left custom_shadow 
           px-4 lg:px-0 "
        >
          Accédez aux bulletins de vote numériques, examinez
          <br className="hidden lg:block" /> les profils des candidats et
          exprimez votre choix en
          <br className="hidden lg:block" /> toute confidentialité, où que vous
          soyez de manière
          <br className="hidden lg:block" /> sûre, pratique et transparente
        </p>
      </div>
      <div className="my-7 lg:my-0">
        <Image src={Illustration_voter} alt="Illustration vote" width="350" />
      </div>
    </div>
  );
}
